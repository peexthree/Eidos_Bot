...
# HexButton needs to show the telegram bot avatar (`hubData[something]`) only WHEN CLICKED
# And the icon itself should be the SVG.
# Also we need to add a 30% darkening layer inside the hexagon.
# Since the hexagon is a background image, we can use `clip-path` or an absolutely positioned div inside with clip-path, OR simply add a background overlay if the hexagon button frame permits it.
# Actually the prompt says: "сделать затемнение внитри гексогоналей плавное процентов 30%"
# And "аватары этих кнопок которые стоят на кнопках файлайди они должны открываться при нажатии этих кнопок ,тоесть при открытии этих кнопок к ним привязывается изображение.а вот изображение кнопок надо изменить... и установить для каждой кнопки соовтествующую SVG иконку"

new_hex_button = """const HexButton = ({ title, iconUrl, tgImageUrl, onClick }) => {
  return (
    <motion.button
...
content = re.sub(r'const HexButton = \(\{ title, iconUrl, onClick \}\) => \{[\s\S]*?return \([\s\S]*?\n\};\n', new_hex_button + '\n', content)

# Now replace the Hub rendering to pass tgImageUrl and open modal
# In Hub component:
# Add state for Modal: const [modalImage, setModalImage] = useState(null);
# handleOfflineAction should take imageUrl and open modal. Or just display popup and open image.
# The user said: "аватары этих кнопок которые стоят на кнопках файлайди они должны открываться при нажатии этих кнопок ,тоесть при открытии этих кнопок к ним привязывается изображение"
# We can just use twa.showPopup and we can show the image? twa.showPopup doesn't support images.
# Let's show a modal inside React with the image.

hub_component_replacement = """const Hub = ({ setView }) => {
  ...
  return (
    <>
      ...
      <div className="relative z-10 flex flex-col h-full w-full max-w-sm mx-auto pt-24 pb-16 px-4">
...
content = re.sub(r'const Hub = \(\{ setView \}\) => \{[\s\S]*?export default Hub;', hub_component_replacement + '\n\nexport default Hub;', content)
