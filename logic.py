...
GAME_GUIDE_TEXTS = {
    ...
    'advanced': (
        "<b>🌌 ПРОДВИНУТЫЕ МЕХАНИКИ</b>\n\n"
        "<b>🔴 ДЕМОН МАКСВЕЛЛА (Аномалия):</b>\n"
        "Встречается на глубине 50+ метров. Предлагает сыграть в кости с вашей жизнью.\n"
        "• <b>Ставка HP:</b> Риск здоровьем ради баффа монет (Перегрузка).\n"
        "• <b>Ставка Лута:</b> Риск буфером ради удвоения ресурсов.\n"
        "<i>Проигрыш накладывает Коррозию (дебафф статов).</i>\n\n"
        "<b>🕶 ТЕНЕВОЙ БРОКЕР:</b>\n"
        "Появляется случайно (2% шанс) при действиях в меню. Продает уникальные артефакты, которых нет в обычном магазине.\n"
        "• Цены могут быть в XP или BioCoins.\n"
        "• Ассортимент меняется каждое появление.\n\n"
        "<b>🔐 ДЕШИФРАТОР КЕЙСОВ:</b>\n"
        "В рейдах можно найти <b>Зашифрованный Кэш</b>. Вскрыть его можно в главном меню.\n"
        "• Требует времени (4 часа). Декодер ускоряет процесс.\n"
        "• Внутри: много XP, монет и шанс на <b>Легендарное снаряжение</b>."
    )
}
...
def process_raid_step(uid, answer=None, start_depth=None):
    try:
        ...
        with db.db_session() as conn:
            with conn.cursor(cursor_factory=db.RealDictCursor) as cur:
                ...
                if not s:
                    ...
                    # Создаем сессию
                    depth = start_depth if start_depth is not None else u.get('max_depth', 0)
        ...
        return False, f"⚠️ КРИТИЧЕСКИЙ СБОЙ: {e}", None, None, 'error', 0
...
def claim_decrypted_cache(uid):
    ...
    # Item Drop (Guaranteed for Cache, high rarity chance)
    import config

    pool_epic = [k for k,v in config.EQUIPMENT_DB.items() if 2000 <= v.get('price', 0) < 10000]
    pool_leg = [k for k,v in config.EQUIPMENT_DB.items() if v.get('price', 0) >= 10000]
    pool_rare = [k for k,v in config.EQUIPMENT_DB.items() if 500 <= v.get('price', 0) < 2000]
    ...
    elif roll < 0.60: # 35% Rare
        if pool_rare: drop_item = random.choice(pool_rare)

    if drop_item:
        db.add_item(uid, drop_item)
        name = config.EQUIPMENT_DB[drop_item]['name']
        msg += f"\n📦 <b>ПРЕДМЕТ:</b> {name}"
