...
def connect_db():
    ...
    try:
        if GOOGLE_JSON:
            ...
            for r in records:
                # Читаем поля. Важно: в гугл таблице должны быть заголовки Type, Path, Text, Level
                ...
                if text:
                    # Если тип "signal", кидаем в общий котел сигналов
                    if r_type == 'signal':
                        CONTENT_DB["signals"].append(text)
                    else:
                        # Иначе распределяем по путям и уровням (Синхрон)
...
def add_xp(uid, amount):
    if uid in USER_CACHE:
        ...
        # Логика Стрика (Начисляем бонус только раз в день при первом входе, но для простоты добавляем к базе)
        # Если юзер активен сегодня, бонус уже учтен или дается за каждое действие? 
        # По ТЗ: "Стрик умножает получение опыта". Сделаем добавку к каждому получению.
...
def start_cmd(m):
    ...
    # Мгновенная регистрация в кэш (чтобы кнопки работали сразу)
