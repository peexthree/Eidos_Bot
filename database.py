...
def fix_user_equipment_schema():
    print("/// DEBUG: Checking user_equipment schema...")
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                # Check for PK
                ...
                # 1. Cleanup NULLs
                cur.execute("DELETE FROM user_equipment WHERE uid IS NULL OR slot IS NULL")
                ...
                conn.commit()
                print("/// FIX: user_equipment schema repaired.")
    except Exception as e:
