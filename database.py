    ...
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_players_level_xp ON players(level, xp);"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_players_max_depth_xp ON players(max_depth, xp);"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_players_biocoin_xp ON players(biocoin, xp);"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_players_total_spent_xp ON players(total_spent, xp);"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_players_protocol_notified_active ON players(last_protocol_time, notified, is_active);"
                )
                # Inventory
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_inventory_uid_item_id ON inventory(uid, item_id);"
                )
                # Equipment
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_user_equipment_uid_slot ON user_equipment(uid, slot);"
                )
                # PVP
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_pvp_logs_target_timestamp_anonymous ON pvp_logs(target_uid, timestamp, is_anonymous);"
                )
                cur.execute(
                    "CREATE INDEX IF NOT EXISTS idx_pvp_logs_attacker_target_timestamp ON pvp_logs(attacker_uid, target_uid, timestamp);"
                )
                conn.commit()
...
TABLE_SCHEMAS = {
    ...
    "user_knowledge": {"uid": ("BIGINT", None), "content_id": ("INTEGER", None)},
    "unlocked_protocols": {"uid": ("BIGINT", None), "protocol_id": ("INTEGER", None)},
    "diary": {
        "id": ("SERIAL", None),
        "uid": ("BIGINT", None),
        "entry": ("TEXT", None),
...
def admin_force_delete_item(uid, item_id):
    with db_cursor() as cur:
        ...
        cur.execute(
            "DELETE FROM inventory WHERE uid = %s AND item_id = %s", (uid, item_id)
        )
        ...
        with db_session() as conn:
            with conn.cursor() as cur:
                ...
                cols_to_fix = [
                    ...
                    "last_signal_time",
                    "anomaly_buff_expiry",
                    "shadow_broker_expiry",
                    "encrypted_cache_unlock_time",
    ...
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                ...
                try:
                    cur.execute(
                        "UPDATE diary SET created_at = CURRENT_TIMESTAMP WHERE created_at IS NULL"
                    )
                    cur.execute(
                        "ALTER TABLE diary ALTER COLUMN created_at SET DEFAULT CURRENT_TIMESTAMP"
                    )
                except Exception as e:
                ...
                try:
                    ...
                    cur.execute(
                        "ALTER TABLE achievements ALTER COLUMN created_at SET DEFAULT CURRENT_TIMESTAMP"
                    )
                except Exception as e:
                ...
                try:
                    cur.execute(
                        "UPDATE history SET reset_date = CURRENT_TIMESTAMP WHERE reset_date IS NULL"
                    )
                    cur.execute(
                        "ALTER TABLE history ALTER COLUMN reset_date SET DEFAULT CURRENT_TIMESTAMP"
                    )
                except Exception as e:
    ...
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "SELECT column_default FROM information_schema.columns WHERE table_name='pvp_logs' AND column_name='id'"
                )
                res = cur.fetchone()
                if res and res[0] is None:
                    ...
                    try:
                        ...
                        if max_id is None:
                            max_id = 0
                        cur.execute(
                            f"SELECT setval('pvp_logs_id_seq', {max_id + 1}, false)"
                        )
                        # Set default
                        cur.execute(
                            "ALTER TABLE pvp_logs ALTER COLUMN id SET DEFAULT nextval('pvp_logs_id_seq')"
                        )
                        # Add PK if missing
                        cur.execute("""
                            SELECT conname FROM pg_constraint
...
def populate_villains():
    with db_session() as conn:
        with conn.cursor() as cur:
            ...
            try:
                cur.execute(
                    "DELETE FROM villains WHERE name IN %s", (OLD_VILLAINS_NAMES,)
                )
            except Exception as e:
...
def add_user(uid, username, first_name, referrer=None):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO players (uid, username, first_name, referrer, last_active, xp, level, biocoin) VALUES (%s, %s, %s, %s, CURRENT_TIMESTAMP, 0, 1, 0) ON CONFLICT (uid) DO NOTHING",
                (uid, username, first_name, referrer),
            )
            if referrer and str(referrer) != str(uid):
                cur.execute(
                    "UPDATE players SET ref_count = ref_count + 1 WHERE uid = %s",
                    (referrer,),
                )
    ...
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "SELECT uid, referrer FROM players WHERE uid = ANY(%s)", (uids,)
                )
    ...
    if cursor:
        ...
        if amount > 0:
            ...
            if res and res[0]:
                ...
                actual_amount = amount - profit
        cursor.execute(
            "UPDATE players SET xp = xp + %s WHERE uid = %s", (actual_amount, uid)
        )
        if profit > 0 and ref_id:
            cursor.execute(
                "UPDATE players SET xp = xp + %s, ref_profit_xp = ref_profit_xp + %s WHERE uid = %s",
                (profit, profit, ref_id),
            )
            cursor.execute(
                "UPDATE players SET generated_ref_xp = generated_ref_xp + %s WHERE uid = %s",
                (profit, uid),
            )
    else:
        with _xp_cache_lock:
...
def increment_user_stat(uid, stat, amount=1, cursor=None):
    ...
    def _execute_logic(cur, uid, amount):
        cur.execute(
            f"UPDATE players SET {stat} = {stat} + %s WHERE uid = %s", (amount, uid)
        )
        return cur.rowcount > 0
...
def reset_daily_stats(uid):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE players SET raid_count_today = 0, last_raid_date = CURRENT_TIMESTAMP WHERE uid = %s",
                (uid,),
            )
...
def use_item(uid, item_id, qty=1, cursor=None):
    ...
    def _use_logic(cur):
        cur.execute(
            "SELECT id, quantity FROM inventory WHERE uid=%s AND item_id=%s ORDER BY id LIMIT 1",
            (uid, item_id),
        )
        row = cur.fetchone()
        if not row:
...
def decrease_durability(uid, slot, amount=1):
    ...
    with db_cursor() as cur:
        if not cur:
            return False
        cur.execute(
            "SELECT durability, item_id FROM user_equipment WHERE uid=%s AND slot=%s",
            (uid, slot),
        )
        res = cur.fetchone()
        if not res:
        ...
        cur.execute(
            "UPDATE user_equipment SET durability = %s WHERE uid=%s AND slot=%s",
            (new_dur, uid, slot),
        )
...
def equip_item(uid, inv_id, slot):
    ...
    try:
        with db_cursor() as cur:
            ...
            cur.execute(
                "SELECT item_id, durability, quantity, custom_data FROM inventory WHERE id=%s AND uid=%s",
                (inv_id, uid),
            )
            res = cur.fetchone()
            if not res:
            ...
            cur.execute(
                "SELECT item_id, durability, custom_data FROM user_equipment WHERE uid=%s AND slot=%s",
                (uid, slot),
            )
            old = cur.fetchone()
            if old:
                ...
                if old_dur is None:
                    old_dur = 10
                cur.execute(
                    "INSERT INTO inventory (uid, item_id, quantity, durability, custom_data) VALUES (%s, %s, 1, %s, %s)",
                    (uid, old_item_id, old_dur, old_custom),
                )
            # 3. Equip new item
    ...
    try:
        with db_cursor() as cur:
            ...
            if durability is None:
                durability = 10
            cur.execute(
                "DELETE FROM user_equipment WHERE uid=%s AND slot=%s", (uid, slot)
            )
            # Move to inventory directly to preserve custom_data
            cur.execute(
                "INSERT INTO inventory (uid, item_id, quantity, durability, custom_data) VALUES (%s, %s, 1, %s, %s)",
                (uid, item_id, durability, custom_data),
            )
            return True
    except Exception as e:
...
def get_equipped_items_full(uid, cursor=None):
    ...
    if cursor:
        cursor.execute(
            "SELECT slot, item_id, durability, custom_data FROM user_equipment WHERE uid=%s",
            (uid,),
        )
...
def break_equipment_randomly(uid):
    ...
    with db_cursor() as cur:
        ...
        cur.execute(
            "SELECT slot, item_id, durability FROM user_equipment WHERE uid=%s ORDER BY RANDOM() LIMIT 1",
            (uid,),
        )
        res = cur.fetchone()
        if not res:
        ...
        if new_dur == 0:
            ...
            cur.execute(
                "SELECT custom_data FROM user_equipment WHERE uid=%s AND slot=%s",
                (uid, slot),
            )
            cd_res = cur.fetchone()
            ...
            cur.execute(
                "INSERT INTO inventory (uid, item_id, quantity, durability, custom_data) VALUES (%s, %s, 1, 0, %s)",
                (uid, item_id, custom_data),
            )
...
def repair_item(uid, inv_id):
    ...
    with db_cursor() as cur:
        if not cur:
            return False
        cur.execute(
            "SELECT item_id FROM inventory WHERE id=%s AND uid=%s", (inv_id, uid)
        )
        res = cur.fetchone()
        if not res:
            return False
        ...
        cur.execute(
            "UPDATE inventory SET durability = %s WHERE id=%s", (max_dur, inv_id)
        )
        return True
...
def dismantle_item(uid, inv_id):
    ...
    with db_cursor() as cur:
        if not cur:
            return False
        cur.execute(
            "SELECT item_id, quantity FROM inventory WHERE id=%s AND uid=%s",
            (inv_id, uid),
        )
        res = cur.fetchone()
        if not res:
        ...
        if qty > 1:
            cur.execute(
                "UPDATE inventory SET quantity = quantity - 1 WHERE id=%s", (inv_id,)
            )
            success = True
        else:
            cur.execute("DELETE FROM inventory WHERE id=%s", (inv_id,))
        ...
        if success:
            ...
            if coins_to_add > 0:
                cur.execute(
                    "UPDATE players SET biocoin = biocoin + %s WHERE uid = %s",
                    (coins_to_add, uid),
                )
    if success:
        cache_db.clear_cache(uid)
...
def check_achievement_exists(uid, ach_id):
    with db_cursor() as cur:
        if not cur:
            return False
        cur.execute(
            "SELECT 1 FROM achievements WHERE uid = %s AND ach_id = %s", (uid, ach_id)
        )
...
def grant_achievement(uid, ach_id, bonus_xp):
    with db_cursor() as cur:
        if not cur:
            return False
        cur.execute(
            "INSERT INTO achievements (uid, ach_id, created_at) VALUES (%s, %s, CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING",
            (uid, ach_id),
        )
        if cur.rowcount > 0:
            cur.execute(
                "UPDATE players SET xp = xp + %s WHERE uid = %s", (bonus_xp, uid)
            )
            return True
        return False
...
def save_knowledge(uid, content_id):
    with db_cursor() as cur:
        if not cur:
            return
        cur.execute(
            "INSERT INTO user_knowledge (uid, content_id) VALUES (%s, %s) ON CONFLICT DO NOTHING",
            (uid, content_id),
        )
        cur.execute(
            "INSERT INTO unlocked_protocols (uid, protocol_id) VALUES (%s, %s) ON CONFLICT DO NOTHING",
            (uid, content_id),
        )
        if cur.rowcount > 0:
            cur.execute(
                "UPDATE players SET know_count = know_count + 1 WHERE uid = %s", (uid,)
            )
...
def add_diary_entry(uid, text):
    with db_cursor() as cur:
        if not cur:
            return
        cur.execute(
            "INSERT INTO diary (uid, entry, created_at) VALUES (%s, %s, CURRENT_TIMESTAMP)",
            (uid, text),
        )
...
def get_diary_entries(uid, limit=5, offset=0):
    with db_cursor(cursor_factory=RealDictCursor) as cur:
        if not cur:
            return []
        cur.execute(
            "SELECT entry, created_at FROM diary WHERE uid = %s ORDER BY created_at DESC LIMIT %s OFFSET %s",
            (uid, limit, offset),
        )
        return cur.fetchall()
...
def get_referrals_stats(uid):
    with db_cursor(cursor_factory=RealDictCursor) as cur:
        if not cur:
            return []
        cur.execute(
            "SELECT username, first_name, level, generated_ref_xp, generated_ref_coins FROM players WHERE referrer = %s ORDER BY generated_ref_xp DESC LIMIT 20",
            (str(uid),),
        )
        return cur.fetchall()
...
def get_random_villain(level=1, cursor=None):
    ...
    max_lvl = level
    query = (
        "SELECT * FROM villains WHERE level BETWEEN %s AND %s ORDER BY RANDOM() LIMIT 1"
    )
    fallback_query = (
        "SELECT * FROM villains ORDER BY ABS(level - %s) ASC, RANDOM() LIMIT 1"
    )
    if cursor:
...
def update_raid_enemy(uid, enemy_id, hp):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE raid_sessions SET current_enemy_id = %s, current_enemy_hp = %s WHERE uid = %s",
                (enemy_id, hp, uid),
            )
...
def clear_raid_enemy(uid):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE raid_sessions SET current_enemy_id = NULL, current_enemy_hp = NULL WHERE uid = %s",
                (uid,),
            )
...
def get_raid_session_enemy(uid):
    with db_cursor(cursor_factory=RealDictCursor) as cur:
        if not cur:
            return None
        cur.execute(
            "SELECT current_enemy_id, current_enemy_hp, is_elite FROM raid_sessions WHERE uid = %s",
            (uid,),
        )
        return cur.fetchone()
    ...
    def _fetch_content():
        with db_cursor(cursor_factory=RealDictCursor) as cur:
            if not cur:
                return []
            cur.execute(
                "SELECT id, type, path, level, text FROM content WHERE type = %s",
                (c_type,),
            )
            return cur.fetchall()

    return (
        cache_db.get_cached_state(f"content_{c_type}", _fetch_content, ttl=300.0) or []
    )
...
def admin_add_content(c_type, text):
    with db_cursor() as cur:
        ...
        if c_type == "raid":
            cur.execute(
                "INSERT INTO raid_content (text, type, val) VALUES (%s, 'neutral', 0)",
                (text,),
            )
        else:
            cur.execute(
                "INSERT INTO content (type, path, text) VALUES (%s, 'general', %s)",
                (c_type, text),
            )
...
def admin_add_signal_to_db(text, level=1, c_type="protocol", path="general"):
    with db_cursor() as cur:
        if not cur:
            return False
        cur.execute(
            "INSERT INTO content (type, path, text, level) VALUES (%s, %s, %s, %s)",
...
def set_user_admin(uid, status):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE players SET is_admin = %s WHERE uid = %s", (status, uid)
            )
...
def is_user_admin(uid):
    ...
    with db_cursor() as cur:
        ...
        res = cur.fetchone()
        return (
            (res[0] if isinstance(res, tuple) else res.get("is_admin"))
            if res
            else False
        )
...
def get_random_user_for_hack(exclude_uid):
    with db_cursor() as cur:
        if not cur:
            return None
        cur.execute(
            "SELECT uid FROM players WHERE uid != %s ORDER BY RANDOM() LIMIT 1",
            (exclude_uid,),
        )
        res = cur.fetchone()
...
def get_death_loot_at_depth(depth):
    with db_cursor(cursor_factory=RealDictCursor) as cur:
        if not cur:
            return None
        cur.execute(
            "SELECT * FROM death_loot WHERE depth = %s ORDER BY created_at DESC LIMIT 1",
            (depth,),
        )
        return cur.fetchone()
...
def log_death_loot(depth, amount, owner_name, cursor=None):
    if cursor:
        cursor.execute(
            "INSERT INTO death_loot (depth, amount, created_at, original_owner_name) VALUES (%s, %s, %s, %s)",
...
def log_action(uid, action, details, cursor=None):
    def _execute_logic(cur, uid, action, details):
        cur.execute(
            "INSERT INTO logs (uid, action, details, timestamp) VALUES (%s, %s, %s, CURRENT_TIMESTAMP)",
            (uid, action, details),
        )
...
def save_raid_grave(depth, loot_json, owner_name, message=""):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO raid_graves (depth, loot_json, owner_name, message, created_at) VALUES (%s, %s, %s, %s, %s)",
...
def get_random_grave(depth):
    ...
    with db_cursor(cursor_factory=RealDictCursor) as cur:
        if not cur:
            return None
        cur.execute(
            "SELECT * FROM raid_graves WHERE depth BETWEEN %s AND %s ORDER BY RANDOM() LIMIT 1",
            (min_d, max_d),
        )
        return cur.fetchone()
...
def save_history(uid, data_json):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO history (uid, archived_data_json, reset_date) VALUES (%s, %s, CURRENT_TIMESTAMP)",
                (uid, data_json),
            )
...
def set_onboarding_stage(uid, stage):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE players SET onboarding_stage = %s WHERE uid = %s", (stage, uid)
            )
...
def add_quiz_history(uid, question_id):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE players SET quiz_history = quiz_history || %s || ',' WHERE uid = %s",
                (str(question_id), uid),
            )
...
def mark_log_revenged(log_id):
    with db_session() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE pvp_logs SET is_revenged = TRUE WHERE id = %s", (log_id,)
            )
...
def update_shadow_metric(uid, metric_name, amount=1):
    ...
    with db_session() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "INSERT INTO user_shadow_metrics (uid) VALUES (%s) ON CONFLICT DO NOTHING",
                (uid,),
            )
            # Update the metric
            cur.execute(
                f"UPDATE user_shadow_metrics SET {metric_name} = {metric_name} + %s WHERE uid = %s",
                (amount, uid),
            )
            conn.commit()
...
def set_shadow_metric(uid, metric_name, value):
    ...
    with db_session() as conn:
        with conn.cursor() as cur:
            ...
            # Set the metric
            cur.execute(
                f"UPDATE user_shadow_metrics SET {metric_name} = %s WHERE uid = %s",
                (value, uid),
            )
            conn.commit()
    ...
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                ...
                if count > 0:
                    print(
                        f"/// DEBUG: Skipping populate_villains (found {count} entries)"
                    )
                    return
                populate_villains()
    except Exception as e:
    ...
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                ...
                if count > 0:
                    print(
                        f"/// DEBUG: Skipping populate_content (found {count} entries)"
                    )
                    return
                populate_content()
    except Exception as e:
...
def admin_clear_all_glitches():
    ...
    with db_cursor() as cur:
        if cur:
            ...
            count = cur.rowcount
            cur.execute(
                "UPDATE user_shadow_metrics SET last_hard_glitch_time = %s",
                (int(time.time()),),
            )
            return count
    return 0
