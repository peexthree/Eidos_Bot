...
def fix_user_equipment_schema():
    print("/// DEBUG: Checking user_equipment table schema...")
    try:
        with db_session() as conn:
            with conn.cursor() as cur:
                # Check if constraint exists
                ...
                print("/// FIX: Adding PRIMARY KEY constraint to user_equipment(uid, slot)...")

                # Remove duplicates first (keep arbitrary one via ctid)
                # Since we don't have a serial ID, we use ctid to distinguish rows
                cur.execute("""
                ...
                conn.commit()
                print("/// FIX: User Equipment schema repair complete.")
    except Exception as e:
