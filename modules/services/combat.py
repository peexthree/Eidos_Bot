...
def perform_hack(attacker_uid):
    ...
    # Cost
    HACK_COST_XP = 50
    if int(atk_u.get('xp', 0) or 0) < HACK_COST_XP:
    ...
    def_score = def_stats['def'] + (int(int(def_u.get('level', 1) or 1)) * 2) + random.randint(1, 20)
    ...
    if has_firewall:
        ...
        # Pay Cost
        db.update_user(attacker_uid, xp=max(0, int(atk_u.get('xp', 0) or 0) - HACK_COST_XP))
        msg = f"🛡 <b>ВЗЛОМ ПРЕДОТВРАЩЕН!</b>\nУ @{def_u['username']} сработал Файрвол."

    elif atk_score > def_score:
        # Steal 5-10% coins
        steal_perc = random.uniform(0.05, 0.10)
        steal_amount = int(int(def_u.get('biocoin', 0) or 0) * steal_perc)
        steal_amount = min(steal_amount, 5000) # Cap
        if steal_amount < 0: steal_amount = 0

        # Transaction
        db.update_user(attacker_uid, biocoin=int(atk_u.get('biocoin', 0) or 0) + steal_amount, xp=int(atk_u.get('xp', 0) or 0) - HACK_COST_XP)
        db.update_user(target_uid, biocoin=max(0, int(def_u.get('biocoin', 0) or 0) - steal_amount))
        ...
        # Penalty: Lose XP
        loss_xp = 100
        db.update_user(attacker_uid, xp=max(0, int(atk_u.get('xp', 0) or 0) - HACK_COST_XP - loss_xp))
