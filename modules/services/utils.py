...
def apply_zalgo_effect(text, intensity=1):
    """Generates Zalgo text (broken font effect) - LIGHTENED."""
    import random as rnd
    zalgo_up = ['̍', '̎', '̄', '̅', '̿', '̑', '̆', '̐', '͒', '͑', '̀', '́', '̋', '̏', '̒', '̓', '̔', '̽', '̉', 'ͣ', 'ͤ', 'ͥ', 'ͦ', 'ͧ', 'ͨ', 'ͩ', 'ͪ', 'ͫ', 'ͬ', 'ͭ', 'ͮ', 'ͯ', '̾', '͛', '͆', '̚']
    zalgo_down = ['̖', '̗', '̘', '̙', '̜', '̝', '̞', '̟', '̠', '̤', '̥', '̦', '̩', '̪', '̫', '̬', '̭', '̮', '̯', '̰', '̱', '̲', '̳', '̹', '̺', '̻', '̼', 'ͅ', '͔', '͕', '͙', '͚', '̣']
    zalgo_mid = ['̕', '̛', '̀', '́', '͘', '̡', '̢', '̧', '̨', '̴', '̵', '̶', '͏', '͜', '͝', '͞', '͟', '͠', '͢', '̸', '̷', '͡', '҉']

    result = ''
    all_zalgo = zalgo_up + zalgo_mid + zalgo_down
    for char in text:
        result += char
        if char.isalnum() and rnd.random() < 0.2: # Only 20% chance to glitch a character
            for _ in range(intensity):
                result += rnd.choice(all_zalgo)
    return result
