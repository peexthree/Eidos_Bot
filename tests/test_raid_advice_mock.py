...
class TestRaidAdvice(unittest.TestCase):
    ...
    @patch('database.get_item_count')
    @patch('database.get_random_villain')
    @patch('database.get_random_raid_advice')
    @patch('logic.generate_random_event_type')
    def test_advice_appears(self, mock_gen_event, mock_get_advice, mock_get_villain, mock_get_item_count, mock_get_user, mock_db_session):
        # Setup mocks
        mock_conn = MagicMock()
        ...
        # Mock raid session (random event)
    ...
    @patch('database.get_random_raid_advice')
    @patch('logic.generate_random_event_type')
    def test_advice_level_logic(self, mock_gen_event, mock_get_advice, mock_get_villain, mock_get_item_count, mock_get_user, mock_db_session):
        # Setup mocks
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_db_session.return_value.__enter__.return_value = mock_conn
